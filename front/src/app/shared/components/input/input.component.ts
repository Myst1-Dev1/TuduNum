...
export class InputComponent implements ControlValueAccessor {
  ...
  label = input<string>('');
  type = input<'text' | 'password' | 'email' | 'number' | 'date' | 'datetime-local'>('text');
  placeholder = input<string>('');
  error = input<string>('');

  inputClass = computed(() => {
    const base = 'w-full px-4 py-3 bg-[#131B2E] dark:bg-dark-card border rounded-xl text-sm transition-all focus:outline-none focus:ring-2 placeholder:text-slate-400 dark:placeholder:text-slate-500 disabled:opacity-50 disabled:cursor-not-allowed';
    const normal = 'border-slate-400 dark:border-dark-border focus:ring-brand-500/20 focus:border-brand-500';
